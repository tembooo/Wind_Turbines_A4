%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Arman Golbidi
% This function performs kernel PCA with RBF kernel, healthy-based scaling.
% - Drop zero-variance-in-healthy columns
% - Scale all rows with healthy μ,σ
% - Fit KPCA on Healthy block only, project all rows
% - Mathematical description: Kernel K(x_i,x_j) = exp(-||x_i - x_j||^2 / (2σ^2))
%   Centered kernel: \tilde{K} = K - 1_n K - K 1_n + 1_n K 1_n, where 1_n = ones(n,n)/n
%   Eigendecomposition: \tilde{K} = V Λ V^T, alpha = V * diag(1/sqrt(λ)), λ = eigenvalues / n
%   Scores for new data: t = \tilde{K}_{test} alpha, where \tilde{K}_{test} is centered kernel between test and train.
...
% KPCA with RBF kernel, healthy-based scaling
% sigma: kernel width for RBF (tune via CV, e.g., 1.5)

    fprintf('running KPCA (RBF, sigma=%.2f) on %d observations and %d variables...\n', sigma, size(data_matrix, 1), size(data_matrix, 2));
    ...
    K = rbf_kernel(Zh, Zh, sigma);  % RBF kernel matrix

    one_n = ones(nh, nh) / nh;
    tilde_K = K - one_n * K - K * one_n + one_n * K * one_n;  % Centered kernel
